function [opt_input, sys] = p4_s2_opt_barrier(sys)
%%
...
while 1
    ...
    record_Q = trace_Q;
    %%
    [SOLu1,SOLu2,SOL1,SOL2,kk] = p4_sos_function_b1(f,k_u,L_au,solh,V,gamma,gg);
    if kk == 0
        break
    end
    Control = [Control; [SOLu1 SOLu2]];
    %%
    [solh,trace_Q,kk] = p4_sos_function_b2(iter,f,k_h,SOLu1,SOLu2,SOL1,SOL2,gamma,V,C,dom,gg,L_us);
    if kk == 0
        break
    end
...
if ~isempty(A)
    ...
    if sys.plott == 1
        ...
        [~,~]=pcontour(opt_input(3),0,domain,'b');
    end
else
    sys.exist_bar = 0;
    sys.perm_b = 0;
