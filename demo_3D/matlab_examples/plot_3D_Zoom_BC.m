...
for iter = 5:length(Barrier)
    ...
    if mod(iter,6) == 0
        set(phB,'EdgeAlpha',0.8,'FaceColor', 'none', 'EdgeColor', k(6),'LineStyle','-','LineWidth',1);
    else
        set(phB,'EdgeAlpha',0.8,'FaceColor', 'none', 'EdgeColor', k(mod(iter,6)),'LineStyle','-','LineWidth',1);
    end
