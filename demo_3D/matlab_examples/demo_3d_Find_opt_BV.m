...
for i = 9:length(B)
    ...
    while 1
        ...
        if mod(iter,2) == 1
            set(inH, 'EdgeAlpha',0.1,'FaceColor', 'none', 'EdgeColor', 'r','LineStyle','-','LineWidth',0.7 ); hold on;
            view(-30,20);hold on;
        else
            set(inH, 'EdgeAlpha',0.1,'FaceColor', 'none', 'EdgeColor', 'g','LineStyle','-','LineWidth',0.7 ); hold on;
            view(-30,20);hold on;
        end
        xlim([-dom dom]); ylim([-dom dom]); zlim([-dom dom]); view(-30,20);
        if kk == 0
