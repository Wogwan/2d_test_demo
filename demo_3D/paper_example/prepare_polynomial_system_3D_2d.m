...
%% The second dimension
f3_p = -x1-2*x2-x3+x2^3;
f3_np = -(exp(2*x3)*cos(x3)^2)^(1/4);
f3 = f3_p + f3_np;
...
subplot(121);hold on;
a1 = plot(y2,'k');hold on;
set(a1,'linewidth',3,'linestyle','-.');
a2 = plot(y_deg_2,'m--');hold on;
...
for num = 1:length(x_initial(:,1))
    dXtr_x_mid_2 = [dXtr_x_mid_2; vpa(subs(f2,[x1,x2,x3],[x_initial(num,1),x_initial(num,2),x_initial(num,3)]))];
...
for num = 1:length(xtest_initial(:,1))
    dXtr_xtest_mid_2 = [dXtr_xtest_mid_2; vpa(subs(f2,[x1,x2,x3],[xtest_initial(num,1),xtest_initial(num,2),x_initial(num,3)]))];
    dXtr_xtest_mid_3 = [dXtr_xtest_mid_3; vpa(subs(f3,[x1,x2,x3],[xtest_initial(num,1),xtest_initial(num,2),x_initial(num,3)]))];
end
%%
y2_mid = reshape(double(dXtr_x_mid_2),1,[])';
y2_test_mid = reshape(double(dXtr_xtest_mid_2),1,[])';
...
dxdt2 = 0;
X = [1];
[mean2,hyp2,delta2,rmse2] = gpr_xdot2_3D(x_initial,y2,xtest_initial,y2_test,it,noise,poly_deg);            % GP learning for the xdot2 dynamic systems
pvar x1 x2 x3
