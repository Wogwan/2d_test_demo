function [h, kk] = sos_optimal_V1_2D_test(f,gg,B,u1,u2,l_au,l_us,h_degree,C,gamma,V)
...
[L7,L7_Q] = sosdecvar('L7_w',monomials(x,0:l_us/2)); 
[L8,L8_Q] = sosdecvar('L8_w',monomials(x,0:l_au/2)); 
%%
hdot = jacobian(h, x1)*(f(1)+gg(1)*u1)+ jacobian(h, x2)*(f(2)+gg(2)*u2);
Bdot = jacobian(B, x1)*(f(1)+gg(1)*u1)+ jacobian(B, x2)*(f(2)+gg(2)*u2);
...
pconstr_2 = hdot-(-L2*Bdot)-Vtol2 >= 0;
pconstr_3 = Vtol2 >= 0;
% pconstr_4 = -hdot-L6*Vdot >= 0;
pconstr_4 = hdot-(-L6*Vdot) >= 0;
pconstr_5 = h-(L7*B) >= 0;
pconstr_6 = hdot-(L8*Bdot) >= 0;
%%
pcr_21 = -h+C(1)*L3 >= 0;
pcr_22 = -h+C(2)*L4 >= 0;
if length(C) == 2
...
pconstr = [pcr_11;pcr_13;pcr_14;pcr_15;pcr_16;pcr_17;pcr_18;pconstr_1;pconstr_2;pconstr_3;pconstr_4;pconstr_5;pconstr_6;pcr_21;pcr_22;pcr_23];
