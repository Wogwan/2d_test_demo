...
% V = 2*x1^4+5*x2^4-2*x1^2*x2^2+2*x2^2+4*x2^2+2*x1*x2;
% V = 2*x1^4-4*x1^2*x2^2+22*x2^4+8.31*x1^2+2.65*x2^2+10*x1*x2;
...
solU = [];
v_c = [];
% figure(21);subplot(212);hold on;[~,~]=pcontour(V,C0,domain,'g'); 
iter = 1;
while abs(double(cc)-double(C0)) >= 1e-2
    iter = iter + 1;
    if iter ~= 1
        C0 = cc;
    end
