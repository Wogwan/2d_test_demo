...
f = [-0.42*x1-1.05*x2-2.3*x1^2-0.5*x1*x2-x1^3;
    1.98*x1+x1*x2];

gu = [0;-u];


v = 1*x1^2+x1*x2+1*x2^2;
e = [0;0];
cc = 0.1;
gamma = 1;
...
%%
% F = [sos(L2), sos(-Vdot - solL1*(vc - v)), sos(-(vc-v)+c1*L2),sos(-(vc-v)+c2*L3),sos(-(vc-v)+c3*L4),sos(-(vc-v)+c4*L5),sos(-(vc-v)+c5*L6), vc>0];
% [sol,vv,QQ] = solvesos(F,-vc,[],[uc;L2c;L3c;L4c;L5c;L6c;vc]);
