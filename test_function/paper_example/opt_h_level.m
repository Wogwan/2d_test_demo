...
% h = -0.8685695224949144*x1^2-2.768882488639843*x1*x2-2.880099449302878*x2^2-3.19856763630857*x1+0.2475554233229784*x2+16.39370414650271;
h = -0.8685695224949144*x1^2-2.768882488639843*x1*x2-2.880099449302878*x2^2-3.19856763630857*x1+0.2475554233229784*x2;
v = h;  
...
%     smrplot(c5s,0,xran,[1000 300],'r-');
    ...
    smrplot(hinit,0,xran,[2000 400],'c-');
    axis(xran)
...
Vdot = jacobian(solV, x1)*(0.1*x1^2+1*x2)+jacobian(solV,x2)*(0.1*x1*x2-0.2*x1+(1+x1^2)*u);
% hdot = jacobian(h, x1)*(0.1*x1^2+1*x2)+jacobian(h,x2)*(0.1*x1*x2-0.2*x1+(1+x1^2)*u);
F = [sos(L1), sos(-Vdot - L1*solV)];
...
% 
% Vdot = jacobian(solV, x1)*(0.1*x1^2+1*x2)+jacobian(solV,x2)*(0.1*x1*x2-0.2*x1+(1+x1^2)*solu);
% % Vdot = jacobian(v, x1)*f(1) + jacobian(v, x2)*f(2);
...
% % F = [sos(L2),sos(L3),sos(L4), sos(-Vdot - solL1*(vc - v)), sos(-(vc-v)+c1*L2), sos(-(vc-v)+c2*L3), sos(-(vc-v)+c2*L4), vc>0];
% F = [sos(L4),sos(L6), sos(-Vdot - solL1*(vc - solV)), sos(-(vc-solV)+c4*L4), sos(-(vc-solV)+c6*L6), vc>0];
...
% solh = value(vc) -solV;
% figure(1);hold on;
% hs = char(sdisplay(solV));
% xran=[-dom dom -dom dom];
% if mod(i,2)==0
%     smrplot(hs,0,xran,[1500 300],'g-');
% else
%     smrplot(hs,0,xran,[1500 300],'m-');        
% end
% axis(xran)
